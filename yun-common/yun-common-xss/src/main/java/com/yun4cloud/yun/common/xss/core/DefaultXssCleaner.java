...
package com.yun4cloud.yun.common.xss.core;

import cn.hutool.core.util.CharsetUtil;
import com.yun4cloud.yun.common.xss.config.YunXssProperties;
import com.yun4cloud.yun.common.xss.utils.XssUtil;
...
public class DefaultXssCleaner implements XssCleaner {
	...
	private final YunXssProperties properties;

	public DefaultXssCleaner(YunXssProperties properties) {
		this.properties = properties;
	}

	private static Document.OutputSettings getOutputSettings(YunXssProperties properties) {
	...
	public String clean(String bodyHtml, XssType type) {
		...
		YunXssProperties.Mode mode = properties.getMode();
		if (YunXssProperties.Mode.escape == mode) {
			// html 转义
			return HtmlUtils.htmlEscape(bodyHtml, CharsetUtil.UTF_8);
		}
		else if (YunXssProperties.Mode.validate == mode) {
			// 校验
